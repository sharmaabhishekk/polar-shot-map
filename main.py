...
ax.grid(color="k", linewidth=1, alpha=1, linestyle="-")
####
##----Plotting----
####
...
for i, row in enumerate(vals[::-1]):
    for j, cell in enumerate(row):
        aux_ax1.text(x=j,
                     ...
                     rotation=25,
                     size=12,
                     color = "k" if cell not in ["Scored", "Missed", "Saved"] else new_results_dict[cell],
                     fontweight = "bold" if type(cell) == str else None
                     )

##Adding figure information and plotting the logo, and key grid and 
fig.text(0.86, 0.601, "metres\nfrom goal", size=8, rotation=25, color="k", va="bottom")
imscatter(0, 4, "images/messi_cropped.png", aux_ax1, zoom=0.3) ##plot image
fig.text(0.05, 0.9, "Profiling players\nacross Europe's top 5 leagues", ha="left", color="k", size=20, va="top", fontweight="bold")
fig.text(0.97, 0.9, f"\n{player} |2014-2018", ha="right", color="k", size=16, fontweight="bold", va="top")
fig.text(0.05, 0.12, f"By {your_name} |Inspired by Peter McKeever", va="center", color="k", size=10)


aux_ax1.text(2, 1.6, "Key", fontweight="bold", size=12, ha="center", va="center")
ax_sub = plot_inset(2, 0.6, aux_ax1, 1.4, color="k") ##plot key plot

for axis in ax1.axis.values():
    axis.line.set(visible=False)
    axis.major_ticks.set(visible=False)
    axis.major_ticklabels.set(visible=False)
aux_ax1.plot([.6, 3.6], [3.3, 3.3], linestyle="dotted", color="k") ##line between column headers and values
